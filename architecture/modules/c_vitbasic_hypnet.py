...
class ViTBasic_HypNet(ComponentAbstract):
    def __init__(
            self,
            tgmodel,
            embedding_dim,
        ...
        for k, v in tgmodel.state_dict().items():
            ...
            if re.search(r'', k):  # Generate all parameters in target model
                k = k.replace(".", "/")
...
class ViTBasic_HypNet_Regressor(ModelAbstract):
    def __init__(self, vitbasic_hypnet_hypparam, tgmodel):
        super().__init__()
        assert vitbasic_hypnet_hypparam["name"] == "vitbasic_hypnet"
        self.network = ViTBasic_HypNet(
            tgmodel=tgmodel,
            embedding_dim=vitbasic_hypnet_hypparam["embedding_dim"],
            hidden_dim=vitbasic_hypnet_hypparam["hidden_dim"],
            n_hidden=vitbasic_hypnet_hypparam["n_hidden"],
        )
        self.init_param()
...
if __name__ == "__main__":
    ...
    tgmodel = ViTBasic_Classifier(vitbasic_hypparam)

    vitbasic_hypnet_hypparam = {  # cifar 10 is 32 * 32
        "name": "vitbasic_hypnet",  # fixed param for now=
        "embedding_dim": 32,
        "hidden_dim": 100,
        "n_hidden": 3,
    }
    hypmodel = ViTBasic_HypNet_Regressor(vitbasic_hypnet_hypparam=vitbasic_hypnet_hypparam, tgmodel=tgmodel)
