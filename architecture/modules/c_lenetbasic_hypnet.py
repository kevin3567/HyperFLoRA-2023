...
# TODO generalize the adapt to basic_hypnet with any network (long term goal)
...
class LeNetBasic_HypNet(ComponentAbstract):
    def __init__(
            self,
            tgmodel,
            embedding_dim,
        ...
        for k, v in tgmodel.state_dict().items():
            ...
            if re.search(r'', k):  # get all weights
                k = k.replace(".", "/")
...
class LeNetBasic_HypNet_Regressor(ModelAbstract):
    def __init__(self, lenetbasic_hypnet_hypparam, tgmodel):
        super().__init__()
        assert lenetbasic_hypnet_hypparam["name"] == "lenetbasic_hypnet"
        self.network = LeNetBasic_HypNet(
            tgmodel=tgmodel,
            embedding_dim=lenetbasic_hypnet_hypparam["embedding_dim"],
            hidden_dim=lenetbasic_hypnet_hypparam["hidden_dim"],
            n_hidden=lenetbasic_hypnet_hypparam["n_hidden"],
        )
        self.init_param()
...
if __name__ == "__main__":
    from architecture.modules.c_lenetbasic import LeNetBasic_Classifier
    ...
    tgmodel = LeNetBasic_Classifier(lenetbasic_hypparam)

    lenetbasic_hypnet_hypparam = {  # cifar 10 is 32 * 32
        "name": "lenetbasic_hypnet",  # fixed param for now=
        "embedding_dim": 32,
        "hidden_dim": 100,
        "n_hidden": 3,
    }
    hypmodel = LeNetBasic_HypNet_Regressor(lenetbasic_hypnet_hypparam=lenetbasic_hypnet_hypparam, tgmodel=tgmodel)
